...
ggplot(data = wdqs_queries, aes(x = length)) +
...
set.seed(0)
wdqs_queries$log10length <- log10(wdqs_queries$length)
clust <- Mclust(wdqs_queries$log10length, 4)
wdqs_queries$cluster <- factor(LETTERS[clust$classification])

# For calculating f(x):
x = seq(min(wdqs_queries$log10length),
        max(wdqs_queries$log10length),
        length.out = 1000)
...
annotations <- data.frame(x = unname(round(10^clust$parameters$mean, 1)),
                          # y = seq(2.5, 5, length.out = length(levels(query_nchars$cluster))),
                          y = rep(0.3, length(clust$parameters$pro)),
                          cluster = factor(levels(wdqs_queries$cluster)))
