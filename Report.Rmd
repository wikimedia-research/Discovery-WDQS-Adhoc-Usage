...
```{r top_referers_daily, as.is = TRUE}
daily_refs <- wdqs_refs %>%
  # Perform the same cleanup procedure as above:
  mutate(url = sub("query.wikidata.org/#.*", "query.wikidata.org/#…QUERY…", url)) %>%
  keep_where(!is.na(url)) %>%
  keep_where(url %notin% c('https://query.wikidata.org', 'https://query.wikidata.org/', 'https://query.wikidata.org/#…QUERY…')) %>%
  mutate(url = smart_trim(url, 50)) %>%
  # Summarize:
  group_by(date, url) %>%
  summarize(total = sum(n)) %>%
  ...
  keep_where(url %in% head(top_referers_overall$url, 11)) %>%
  tidyr::spread(url, total, fill = 0) %>%
